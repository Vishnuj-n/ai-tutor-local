...
## 13. Required Phase 2 Mode: Teacher Shared Content Packs

This mode is required for Phase 2 scope and should be implemented after P0/P1 sync endpoints are stable.

Rules:
- Teacher publishes a read-only content pack to a class.
- Student imports it as a new local notebook snapshot/copy.
- No live collaborative editing between teacher source and student copy.
- Private student notebooks remain inaccessible to teachers unless represented via allowed telemetry fields.

When implementing this mode, align with required sections in:
- `doc/DATA_API.md`
- `doc/SCHEMA.md`
